...
#include "dust_com.h"
#include "timestamp.h"
#include "rmad.h" // Get logging_is_running()
#include "process.h"
...
#define FLASH_W29N01HV_PAGE_NSCANS (FLASH_W29N01HV_PAGE_SIZ/sizeof(scan_t))
#define FLASH_W29N01HV_BLOCK_EFFECTIVE_NSCANS ((FLASH_W29N01HV_BLOCK_SIZE/FLASH_W29N01HV_PAGE_SIZ - FLASH_HEADER_PAGES)* FLASH_W29N01HV_PAGE_NSCANS)
#define PROCESS_W29N01HV_NSCANS (FLASH_W29N01HV_PAGE_NSCANS * PROCESS_NPAGES)

#define W29N01HV_NUM_SCANS_IN_AUX PROCESS_NSCANS // set equal to used with ould flash, no to alter reporting interval for AVALANCHE_PACKET

//#define FLASH_W29N01HV_INNER_BUFSIZE (10*PROCESS_W29N01HV_NSCANS) // #Scans. Does not have to be n*PROCESS_NSCANS, but most efficient.
        ...
        if (num_scans > 0) {
			...
			if (flash_block_to_use() >= 0) {
				...
				for (int ipage = 0; ipage < npages; ++ipage) {
					...
					//printf("%d, %d, %d, %d, %d, %d\n", nscans_since_traceset_start, FLASH_BLOCK_EFFECTIVE_NSCANS, FLASH_PAGE_NSCANS, ipage, rb_istart, npages);
					...
					if (end_of_traceset || rb->timestamp_is_correct && last_page_in_block_just_written) {
...
/*int process_W29N01HV_ringbuf_entries()
{

	scan_t buf[PROCESS_W29N01HV_NSCANS];
	unsigned int num_scans, rb_istart, rb_num, rrb_istart;
	bool rb_locked;
	bool check_for_more_data;
	scan_ringbuf_t *rb;
	do {
		...
		if (rb_num >= PROCESS_W29N01HV_NSCANS){
			//printf("\nrb_num >= PROCESS_W29N01HV_NSCANS");
		    num_scans = PROCESS_W29N01HV_NSCANS;
		}else if (rb_locked){
			//printf("\nrb_locked");
			num_scans = rb_num; // New traces have already been started, finish current.
		}else{
        ...
        INT_Enable();
        	...
        	//printf("\n!!!!!!!!!!num_scans=%u, rb_locked=%u, rb_num=%u, gl_rrb.num=%u\n", num_scans, rb_locked, rb_num, gl_rrb.num);
        ...
        if (num_scans > 0) {
			...
			// Write to W29N01HV NAND FLASH.

			// This entire function should possibly be re-written for HW >= 6 in order to separate realtime prosessing for storing data to NAND
			...
			if (flash_block_to_use() >= 0) {
				...
				const int npages = ceil((float)num_scans / FLASH_W29N01HV_PAGE_NSCANS);
				//TESTER MINNEFEIL
				//const int npages = ceil((float)num_scans / FLASH_PAGE_NSCANS);

				//printf("\nnumscans %d, num_W29N01HV_scans %d, npages %d\n", num_scans, num_scans, npages);

				for (int ipage = 0; ipage < npages; ++ipage) {
					const unsigned int nscans_since_traceset_start = rb_istart + ipage * FLASH_W29N01HV_PAGE_NSCANS; // at start of page
					const unsigned int traceset_id = rrb_istart % 65536;
					gl_latest_flash_id = traceset_id;
					const unsigned int block_seq_num_in_traceset = nscans_since_traceset_start/(FLASH_W29N01HV_BLOCK_EFFECTIVE_NSCANS); // Integer division intentional
					const bool start_of_new_block = nscans_since_traceset_start % FLASH_W29N01HV_BLOCK_EFFECTIVE_NSCANS == 0;
					const bool end_of_traceset = rb_locked && rb_num == num_scans && ipage == npages-1;
					...
					//printf("flash_W29N01HV_safe_write_page(%d, %d, %d) - %d, %d, %d\n", gl_next_page_in_block_to_use +1, buf + ipage * FLASH_W29N01HV_PAGE_NSCANS, start_of_new_block);

					if (flash_W29N01HV_safe_write_page(gl_next_page_in_block_to_use++, buf + ipage * FLASH_W29N01HV_PAGE_NSCANS, start_of_new_block)	< 0)
						break;

					const bool last_page_in_block_just_written = nscans_since_traceset_start % FLASH_W29N01HV_BLOCK_EFFECTIVE_NSCANS == (FLASH_W29N01HV_BLOCK_EFFECTIVE_NSCANS - FLASH_W29N01HV_PAGE_NSCANS);

					//printf("%d, %d, %d, %d, %d, %d, %d, %d, %d\n", nscans_since_traceset_start, FLASH_W29N01HV_BLOCK_EFFECTIVE_NSCANS, FLASH_W29N01HV_PAGE_NSCANS, ipage, rb_istart, npages, num_scans, end_of_traceset, last_page_in_block_just_written);

					//printf("If %d == %d\n", nscans_since_traceset_start % FLASH_W29N01HV_BLOCK_EFFECTIVE_NSCANS, (FLASH_W29N01HV_BLOCK_EFFECTIVE_NSCANS - FLASH_W29N01HV_PAGE_NSCANS));
						...
						for (int nsteps_back = 0; nsteps_back < FLASH_NBLOCKS; ++nsteps_back) { // Each step is one block
							...
							flash_block_header_t flash_block_header =
								{ FLASH_MAGIC_NUMBER,
								  ...
								  seq_num,
								  (nsteps_back == 0)
								      ? (nscans_since_traceset_start + EFM32_MIN(num_scans - ipage*FLASH_W29N01HV_PAGE_NSCANS, FLASH_W29N01HV_PAGE_NSCANS) - block_seq_num_in_traceset * FLASH_BLOCK_EFFECTIVE_NSCANS)
								      : FLASH_W29N01HV_BLOCK_EFFECTIVE_NSCANS,
                                  TIMESTAMP_ADD_SECS(rb->start_time, ((double)seq_num*FLASH_W29N01HV_BLOCK_EFFECTIVE_NSCANS)/gl_ad_scan_rate),
                                  rb->start_time,
                                  rb->timestamp_is_correct
								};
							// Messy, and code worked well without it, but we want to make sure the flash code
							...
							struct {
							    flash_block_header_t header;
							    unsigned char padding[FLASH_W29N01HV_PAGE_SIZ - sizeof(flash_block_header_t)];
							} flash_block_header_buf = { flash_block_header };
		...
		rb->num -= num_scans;
		check_for_more_data =
		    rb->num >= PROCESS_W29N01HV_NSCANS // Plenty of data in current traceset
			|| rb->num > 0 && rb->locked // Some data left in current traceset, and it is locked so no point in waiting for more
			|| rb->num == 0 && gl_rrb.num > 1; // Finished with current traceset (both reading from and writing to)
		                                       // but at least one more traceset is available that might have data.
		                                       // We don't check whether it actual has data here, rather in the next loop iteration, but we could have.
...
int process_W29N01HV_hybrid_ringbuf_entries()
{
...
//}
			...
			if (flash_block_to_use() >= 0) {
				...
				for (int ipage = 0; ipage < npages; ++ipage) {
					...
					if (flash_W29N01HV_safe_write_page(gl_next_page_in_block_to_use++, buf + ipage * FLASH_PAGE_NSCANS, start_of_new_block)	< 0)
						break;
...
{
	//if(gl_debug_on_UART1)printf("\nprocess_ringbuf_lock_traceset() started");

	if (gl_rrb.num > 0)
		gl_rrb.buf[(gl_rrb.istart + gl_rrb.num - 1) % FLASH_OUTER_BUFSIZE].locked = true;

	//if(gl_debug_on_UART1)printf("\nprocess_ringbuf_lock_traceset() finished");
}
...
{
	//if(gl_debug_on_UART1)printf("\nprocess_ringbuf_add_scan() starting"); - too slow to include
...
#ifdef ALLOW_CALL_PROCESS_RINGBUF_ENTRIES
		printf("ALLOW_CALL_PROCESS_RINGBUF_ENTRIES is defined, process_ringbuf_entries() called outside main() ")
	...
	//if(gl_debug_on_UART1)printf("\nprocess_ringbuf_add_scan() finished"); - too slow to include
}
...
void process_hybrid_send_data(const packet_with_meta_t *const packet_with_meta)
{
    ...
    while (true) {
            ...
            while (true) {
                ...
                if (ipage_in_block == block_ndatapages_used + 1) {  // hvis iscan_start > 64 så begynner denne algoritmen og returnere feil verdier. Regner med at dette henger sammen med at de første dataenen da ikke er fra iblock = 1
                    ...
                    if (HW_REVITION >= 6)
                         {
                        while (true)  {
                            iblock = (iblock + 1) % FLASH_W29N01HV_NBLOCKS;
                            const block_info_t *bl;
                            bl = gl_block_info + iblock;
                            read_ok = false;
                            ...
                            if (!is_initialization && iblock == iblock_current
                                || (bl->status != BLOCK_BAD
                                    && (bl->status != BLOCK_USED
                                        || bl->id != data_request.id
                                        || (read_ok =  read_status == NANDFLASH_W29N01HV_STATUS_OK)))){
                            			//|| (read_ok = 1)))){
                            	//if(gl_debug_on_UART1)printf("\nread_ok_A %d", read_ok);
                            	break;
                            }
        ...
        if (hole_or_similar || packet_full) {
            ...
            if (data_end > 0) {
            	status = send_data(iscan_handled_end - data_end, iscan_handled_end, &data_desc, nchans, &data[0][0]);
            	//if(gl_debug_on_UART1)printf("\ndata_end > 0 - scan_start %d, scan_end %d",iscan_handled_end - data_end, iscan_handled_end);
                data_end = 0;
            }
            if (hole_or_similar) {
            	status = send_data(iscan_handled_end, iscan_after_any_missing, &data_desc, nchans, 0);
            	//if(gl_debug_on_UART1)printf("\nhole_or_similar - scan_start %d, scan_end %d",iscan_handled_end, iscan_after_any_missing);
                iscan_handled_end = iscan_after_any_missing;
            }
        }
        ...
        if (iscan_handled_end >= iscan_request_stop + 1) {
            ...
            //wait(1000); //testing to see if somthing need to finish
...
void process_config()
{
	...
	if (HW_REVITION >= 6)
	{
		...
		// Sanity check(s).
		//assert(FLASH_W29N01HV_PAGE_SIZ % sizeof(scan_t) == 0);
		//assert(PROCESS_W29N01HV_NSCANS % FLASH_W29N01HV_PAGE_NSCANS == 0);

		//if(gl_debug_on_UART1)printf("\nflash_W29N01HV_init() starting");

	    //gl_traceset_id_to_use =
		gl_rrb.istart = flash_W29N01HV_init()+1; // So that gl_rrb.istart can be used as an ID

		//if(gl_debug_on_UART1)printf("\nflash_W29N01HV_init() finished");
