...
NANDFLASH_W29N01HV_Info_TypeDef *gl_W29N01HV_flash_info;// OK X.
...
const NANDFLASH_W29N01HV_Info_TypeDef *flash_W29N01HV_info()
{
	return gl_W29N01HV_flash_info;
}
...
{
	//printf("flash_W29N01HV_write_page()");

	const uint32_t block_addr = FLASH_W29N01HV_PAGE_ADDR(gl_next_block_to_use, ipage_in_block);


			if (NANDFLASH_W29N01HV_EraseBlock(block_addr) == NANDFLASH_W29N01HV_WRITE_ERROR) {
				NANDFLASH_W29N01HV_MarkBadBlock(block_addr);
				gl_block_info[iblock].status = BLOCK_BAD;
...
{
	//printf("flash_W29N01HV_safe_write_page()");
	// We buffer a whole block:

	//static unsigned char block[FLASH_W29N01HV_BLOCK_SIZE];
	//static unsigned char is_written_to[FLASH_W29N01HV_BLOCK_NPAGES]; // bool is 4 bytes in current implementation
	//TESTING AV MINNEFEIL
	static unsigned char block[FLASH_BLOCK_SIZE];  //bruker pagesize fra gammel NAND for å ikke øke lengden mellom pakker (3/4 av NAND vil ikke benyttes)
	static unsigned char is_written_to[FLASH_W29N01HV_BLOCK_NPAGES]; ///bruker blocksize til ny NAND for å benytte få riktig addresering, benytte hele blokka
    ...
    //memcpy(block+ipage_in_block*FLASH_W29N01HV_PAGE_SIZ, page, FLASH_W29N01HV_PAGE_SIZ);
    //TESTING AV MINNEFEIL
    memcpy(block+ipage_in_block*FLASH_PAGE_SIZ, page, FLASH_PAGE_SIZ);
	...
	//printf("\ngl_next_block_to_use %d \n", gl_next_block_to_use);
	...
	bool flash_error;
	do {
		const uint32_t block_addr = FLASH_W29N01HV_PAGE_ADDR(gl_next_block_to_use, 0);
		flash_error = start_new_block && NANDFLASH_W29N01HV_EraseBlock(block_addr) == NANDFLASH_W29N01HV_WRITE_ERROR;

		//printf( "\nflash_erase_error %ld", flash_error);
...
int flash_init()
{
	...
	// Sanity checks.
		...
		if(gl_debug_on_UART1){
			...
			if(gl_debug_on_UART1)printf( "\n  Manufacturer Code :  0x%02X", gl_flash_info->manufacturerCode );
			if(gl_debug_on_UART1)printf( "\n  Device Code       :  0x%02X", gl_flash_info->deviceCode );
			if(gl_debug_on_UART1)printf( "\n  Device size       :  %ld (%ldMB)", gl_flash_info->deviceSize,gl_flash_info->deviceSize/1024/1024 );
			if(gl_debug_on_UART1)printf( "\n  Page size         :  %ld", gl_flash_info->pageSize );
			if(gl_debug_on_UART1)printf( "\n  Spare size        :  %ld", gl_flash_info->spareSize );
			if(gl_debug_on_UART1)printf( "\n  Block size        :  %ld", gl_flash_info->blockSize );
	      //printf( "\n  FLASH_SIZ         :  %ld", FLASH_SIZ );
	      //p1rintf( "\n  FLASH_BLOCK_SIZE  :  %ld", FLASH_BLOCK_SIZE );
	      //printf( "\n  FLASH_PAGE_SIZ    :  %ld", FLASH_PAGE_SIZ );
	    	if(gl_debug_on_UART1)putchar( '\n' );
		}
	...
	for (unsigned int iblock = 0; iblock < FLASH_NBLOCKS; ++iblock) {
        ...
        if (is_bad) {
			...
			//printf( "\nflash_W29N01HV_init() block used = %lx \n", addr);

			//printf( "sizeof(flash_block_header_t) = %ld \n", sizeof(flash_block_header_t));
			//printf( "sizeof(header) = %ld \n", sizeof(header));
			//printf( "sizeof(buffer) = %ld \n", sizeof(buffer));

			//printf( "MAGIC_NUMBER = %1x \n", FLASH_MAGIC_NUMBER);
			//printf( "FLASH_MAGIC_NUMBER = %1x \n", header.magic_number);
			//printf( "FLASH_latest_id = %ld \n", header.id);
    ...
    if(gl_debug_on_UART1)printf( "\nRMAD - Latest Used ID = %ld \n",latest_used_id);

    return latest_used_id;
}

int flash_W29N01HV_init()
{

	/* Setup EBI for NAND Flash. */
	BSP_EbiInit();
	NANDFLASH_W29N01HV_Init(5); // DMA channel 5 used in example, so we use it.
	gl_W29N01HV_flash_info = NANDFLASH_W29N01HV_DeviceInfo();// OK X.

	// Sanity checks.
	/*	assert(FLASH_W29N01HV_SIZ == gl_W29N01HV_flash_info->deviceSize);
		assert(FLASH_W29N01HV_BLOCK_SIZE == gl_W29N01HV_flash_info->blockSize);
		assert(FLASH_W29N01HV_PAGE_SIZ == gl_W29N01HV_flash_info->pageSize);
		assert(FLASH_W29N01HV_SIZ % FLASH_W29N01HV_BLOCK_SIZE == 0);
		assert(FLASH_W29N01HV_BLOCK_SIZE % FLASH_W29N01HV_PAGE_SIZ == 0); */

		if(gl_debug_on_UART1){

			if(gl_debug_on_UART1)printf( "\nNAND flash device information:\n" );
			if(gl_debug_on_UART1)printf( "\n  Manufacturer Code :  0x%02X", gl_W29N01HV_flash_info->manufacturerCode );
			if(gl_debug_on_UART1)printf( "\n  Device Code       :  0x%02X", gl_W29N01HV_flash_info->deviceCode );
			if(gl_debug_on_UART1)printf( "\n  Device size       :  %ld (%ldMB)", gl_W29N01HV_flash_info->deviceSize,gl_W29N01HV_flash_info->deviceSize/1024/1024 );
			if(gl_debug_on_UART1)printf( "\n  Page size         :  %ld", gl_W29N01HV_flash_info->pageSize );
			if(gl_debug_on_UART1)printf( "\n  Spare size        :  %ld", gl_W29N01HV_flash_info->spareSize );
			if(gl_debug_on_UART1)printf( "\n  Block size        :  %ld", gl_W29N01HV_flash_info->blockSize );
	      //printf( "\n  FLASH_W29N01HV_SIZ         :  %ld", FLASH_W29N01HV_SIZ );
	      //printf( "\n  FLASH_W29N01HV_BLOCK_SIZE  :  %ld", FLASH_W29N01HV_BLOCK_SIZE );
	      //printf( "\n  FLASH_W29N01HV_PAGE_SIZ    :  %ld", FLASH_W29N01HV_PAGE_SIZ );
			if(gl_debug_on_UART1)putchar( '\n' );
		}


	unsigned char buffer[EFM32_MAX(gl_W29N01HV_flash_info->spareSize, gl_W29N01HV_flash_info->pageSize)];

	//printf( "\nBuffersize = %ld \n",EFM32_MAX(gl_W29N01HV_flash_info->spareSize, gl_W29N01HV_flash_info->pageSize));

	unsigned int addr = gl_W29N01HV_flash_info->baseAddress;
	int latest_used_block = -1,
		latest_used_id = -1,
		latest_used_seq_num = -1;

	//printf( "\nflash_W29N01HV_init() baseAddress = %lx \n", addr);

	//printf( "\nFLASH_W29N01HV_NBLOCKS = %ld \n",FLASH_W29N01HV_NBLOCKS);

	for (unsigned int iblock = 0; iblock < FLASH_W29N01HV_NBLOCKS; ++iblock) {

		int readspare_error = NANDFLASH_W29N01HV_ReadSpare(addr, buffer); // Peeked in the implementation, seems safe to ignore return value
        ...
        if (is_bad) {
        	...
        	if(gl_debug_on_UART1)printf( "\nflash_W29N01HV_init() bad_block = %lx", addr);
        }
        else if (NANDFLASH_W29N01HV_ReadPage(addr, buffer) != NANDFLASH_W29N01HV_STATUS_OK) {

        	//printf( "\nflash_W29N01HV_init() not used = %lx \n", addr);
        ...
        //addr += gl_W29N01HV_flash_info->blockSize;

        addr += FLASH_W29N01HV_BLOCK_ADRESS_SIZ;
	}
