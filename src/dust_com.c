...
void dust_check_and_possibly_reset_mote()
{
    ...
    if(gl_debug_on_UART1)printf("\ndust_check_and_possibly_reset_mote() status - %d",ok);
    ...
    if (!ok) {
        ...
        //have observed infinite reboot cycle when wakin up for battery saving mode - NVIC_SystemReset() feels like a more safe solution at this point.
			...
			switch (256*rx_data[5] + rx_data[6])
			{
				...
				case 0x0020: /*mote operational and ready to send data*/
					if (dust_open_sockets() < 0){
						...
						if(gl_debug_on_UART1)printf("\nSmartMesh IP mote status - Active - no open sockets");
						...
						// det ser ut til at det er denne funksjonen som kan ha skapet reboot løkker av mote.
						// tester videre ved å disable comp interupt ved boot og mest mote er lost

						//oppdatering - denne funksjonen gir reboot løkker selv med int disablet - tester videre med debud på UART1
						//oppdatering - det ser ut til at det er dust_open_sockets() funksjonen som feiler!!! satt inn NVIC_SystemReset(); i stedet for dust_mote_reset(); for videre testing

				    	//testingen indikerer at reboot løkken oppstår når EFM32 scanner AD kanalene mens SmartMesh IP går lost og så prøver å komme online igjen. Dette vil typisk skje hvis trigg nivået er for nærme bakgrunnsstøyen og radiokaliteten er lav.
				    	//det er mest sannsynlig dust_open_sockets() som ikke får åpnet en port i dette tilfellet. Ved å endre fra dust_mote_reset() til NVIC_SystemReset() omgår vi denne løkka. feilen må derfor ligge i at koden til Inge ikke rydder opp ordentlig etter seg.
				    	//må finne en ordentlig fiks på ett senere tidspungt

					} else {

						if(gl_debug_on_UART1)printf("\nSmartMesh IP mote status - Active");
				...
				case 0x0100: /*mote started joining the network*/
					if(gl_debug_on_UART1)printf("\nSmartMesh IP mote status - Joining");
						gl_mote_online_check = false;
						dust_close_any_sockets();
						wait(10);
